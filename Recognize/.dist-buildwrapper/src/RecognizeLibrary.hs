...
recognize values event = if ( isInRanges values ( eventRanges( event ) ) )
                         ...
                         else if ( eventNFlag( eventNext( event ) ) )
                         ...
                         -- if the case cannot be recognized i.e. this is the last known case -> crate new group
...
createNewCase values event  = do
          ...
          --new_name    = ( groupName( group ) + 1 )
          let newX1Bounds = Bounds ( ( x1( values ) ) - 20/100 * ( x1( values ) ) ) ( ( x1( values ) ) + 20/100 * ( x1( values ) ) )
          let newX2Bounds = Bounds ( ( x2( values ) ) - 20/100 * ( x2( values ) ) ) ( ( x2( values ) ) + 20/100 * ( x2( values ) ) )
          let newX3Bounds = Bounds ( ( x3( values ) ) - 20/100 * ( x3( values ) ) ) ( ( x3( values ) ) + 20/100 * ( x3( values ) ) )
          let newX4Bounds = Bounds ( ( x4( values ) ) - 20/100 * ( x4( values ) ) ) ( ( x4( values ) ) + 20/100 * ( x4( values ) ) )
          let newRange    = Ranges newX1Bounds newX2Bounds newX3Bounds newX4Bounds
          let newEvent    = Event newName newRange True event
          return ( newEvent )

--___________________________________________________________________________________________________________
-- showLastEvent TEST FUNCTION
showLastEvent event = if ( eventNFlag( event ) )
                 then do
                        showLastEvent ( eventNext( event ) )
                 else do 
                        return ( event )
